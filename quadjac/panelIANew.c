...
void intgrnonsing(int d, int p, double h, panel *pX0, panel *pX1, panel *pY0, panel *pY1, double xLeg[], double wLeg[], double solution[]) {
  double *v00, *v10, *v01, *v11, *w00, *w10, *w01, *w11, v0t[2], v1t[2], w0t[2], w1t[2];
  double r[2], ny[2];
  double x, y, t, tau, del, jac, fcnVal[2], dfcn, fcn, xdiff, ydiff, norma, normb, nVeloc, dw0[2], shape[4], h2;
  int i, i1, i2, i3, i4;
  ...
  for(i3 = 0; i3 < p; i3++) {
    ...
    for(i4 = 0; i4 < p; i4++) {
      ...
      if ( del>TOL ) {
        for(i2 = 0; i2 < p; i2++) {
          y = xLeg[i2];
          nVeloc  = (dw0[0]*(1 - y) + dw0[0]*y)*ny[0];
          nVeloc += (dw0[1]*(1 - y) + dw0[1]*y)*ny[1];
          nVeloc /= h;
          for(i1 = 0; i1 < p; i1++) {
            x = xLeg[i1];
            r[0]  = v0t[0]*(1 - x) + v1t[0]*x - (w0t[0]*(1 - y) + w1t[0]*y);
            r[1]  = v0t[1]*(1 - x) + v1t[1]*x - (w0t[1]*(1 - y) + w1t[1]*y);
 
            //heat kernel
            kernel(r, del, ny, fcnVal);
            fcnVal[1] -= nVeloc*fcnVal[0];
            jac = norma*normb;
            jac *= wLeg[i1]*wLeg[i2]*wLeg[i3]*wLeg[i4];
	    fcn = fcnVal[0]*jac;
            dfcn = fcnVal[1]*jac; 
...
void intgrd0nc1(int d, int p, double h, panel *pX0, panel *pX1, panel *pY0, panel *pY1, double xLeg[], double wLeg[], double solution[], int nc) {
  ...
  for(i1 = 0; i1 < p; i1++) { //xi1 loop
	...
	for(i4 = 0; i4 < p; i4++) { //sigma loop
          ...
          nVeloc  = (dw0[0]*(1-eta[i3]*xi1[i1]) + dw0[0]*eta[i3]*xi1[i1])*ny[0];
          nVeloc += (dw0[1]*(1-eta[i3]*xi1[i1]) + dw0[1]*eta[i3]*xi1[i1])*ny[1];
          nVeloc /= h;
          kernel(r, del, ny, fcnVal);
          fcnVal[1] -= nVeloc*fcnVal[0];
          fcn1 = fcnVal[0]*jac;
          ...
          del = h*etasq*xi2sq;
          jac = norma*normb*h*h*2*eta[i3]*xi2[i2]*etasq*(1-etasq*xi2sq);
          nVeloc  = (dw0[0]*(1-eta[i3]) + dw0[0]*eta[i3])*ny[0];
          nVeloc += (dw0[1]*(1-eta[i3]) + dw0[1]*eta[i3])*ny[1];
          nVeloc /= h;
          kernel(r, del, ny, fcnVal);
          fcnVal[1] -= nVeloc*fcnVal[0];
          fcn2 = fcnVal[0]*jac;
          ...
          nVeloc  = (dw0[0]*(1-eta[i3]*xi2[i2]) + dw0[0]*eta[i3]*xi2[i2])*ny[0];
          nVeloc += (dw0[1]*(1-eta[i3]*xi2[i2]) + dw0[1]*eta[i3]*xi2[i2])*ny[1];
          nVeloc /= h;
          kernel(r, del, ny, fcnVal);
          fcnVal[1] -= nVeloc*fcnVal[0];
          fcn3 = fcnVal[0]*jac;
